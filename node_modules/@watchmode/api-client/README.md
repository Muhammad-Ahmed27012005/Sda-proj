...
The SDK handles authentication from the `apiKey` value you pass to `WatchmodeClient`.

For direct REST API calls outside the SDK, send your key in the `X-API-Key` header. `Authorization: Bearer <api-key>` is also supported. The `apiKey` query parameter remains available for backwards compatibility, but headers are recommended for new integrations because URLs are commonly stored in logs, browser history, analytics, and referrer headers.
